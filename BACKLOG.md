...
|---|---|---|---|---|
| 5.1 | **Automated tests** | 🟡 | L | 128 tests (`npm test`): the permission matrix, the integrity rules, the domain vocabulary including a fully customised pipeline, and the action-layer rules — offer state machine, hire cascade, panel verdict, interview-stage sync and stage backfill — which are now pure functions in `src/server/rules.ts` rather than logic buried where nothing could reach it. Still missing: end-to-end tests that exercise an action through a real session. |
