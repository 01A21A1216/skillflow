...
export default async function PipelinePage({
  ...
  const cards = pipelineCards(filters);
  const facets = requisitionFacets();
  const openReqs = listRequisitions({ status: "active", sort: "pipeline" });
      ...
      <PageBody>
        ...
        {cards.length === 0 ? (
          ...
          <PipelineOptionsProvider
            value={{
              interviewers: people
                .filter((p) => ["interviewer", "hiring_manager", "recruiter", "admin"].includes(p.role))
