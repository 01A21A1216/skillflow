...
export default async function OffersPage({
  ...
  const rows = listOffers(filters);
  const stats = offerStats();
  const trend = offerTrend(12);
  const ready = offerReadySubmissions().slice(0, 40);
  const facets = requisitionFacets();
  const openReqs = listRequisitions({ status: "active" });
  ...
  return (
    <>
      <PageHeader
        ...
        actions={<NewOfferButton candidates={ready} />}
        tabs={
          <LinkTabs
            param="status"
            tabs={TABS.map((t) => ({
